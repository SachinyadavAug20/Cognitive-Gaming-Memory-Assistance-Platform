...
import { CalendarCheck, CheckCircle2, Clock, Volume2 } from "lucide-react";
...
interface RoutineAppointmentCardProps {
  ...
  onSpeakAppointment: () => void;
}

export function RoutineAppointmentCard({
  appointmentDone,
  aptTag,
  aptTimeSub,
  aptTitle,
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs font-bold text-ink-secondary mt-0.5">{aptTimeSub}</div>
            </div>
          </div>

          <button
            type="button"
            onClick={onSpeakAppointment}
        ...
        {appointmentDone ? (
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 text-xs font-black text-purple-950 bg-purple-100/90 px-3 py-2.5 rounded-xl border border-purple-300 flex-1 min-w-0 truncate">
              ...
              <span className="truncate">{aptScheduled}</span>
            </div>
            <button
              type="button"
              onClick={onToggleAppointment}
