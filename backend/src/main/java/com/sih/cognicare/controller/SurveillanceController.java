...
public class SurveillanceController {
    ...
    @PostMapping({"/api/v1/surveillance/patients/{id}/readings"})
    public ResponseEntity<SurveillanceReadingDTO> recordReading(
            @PathVariable("id") Long patientId,
            @RequestBody @jakarta.validation.Valid SurveillanceReadingRequest request) {
        return ResponseEntity.ok(surveillanceService.recordReading(patientId, request));
    }

    @PostMapping({"/api/v1/surveillance/patients/{id}/alerts"})
    ...
    @PostMapping({"/api/v1/surveillance/patients/{id}/sos"})
    ...
    @GetMapping({"/api/v1/surveillance/patients/{id}/sos/latest"})
    ...
    @GetMapping({"/api/v1/admin/surveillance/patients"})
    ...
    @GetMapping({"/api/v1/admin/surveillance/patients/{id}"})
    ...
    @GetMapping({"/api/v1/admin/surveillance/alerts"})
    ...
    @PostMapping({"/api/v1/admin/surveillance/alerts/{id}/resolve"})
    ...
    @GetMapping({"/api/v1/admin/surveillance/sos"})
    ...
    @PostMapping({"/api/v1/admin/surveillance/sos/{id}/acknowledge"})
    ...
    @GetMapping({"/api/v1/admin/surveillance/patients/{id}/history"})
    public ResponseEntity<List<SurveillanceReadingDTO>> history(
            @PathVariable("id") Long patientId,
            @RequestParam(value = "hours", defaultValue = "24") int hours) {
        return ResponseEntity.ok(surveillanceService.getHistory(patientId, Math.min(hours, 168)));
    }

    // ── Demo seeder ──

    @PostMapping({"/api/v1/admin/surveillance/simulate"})
