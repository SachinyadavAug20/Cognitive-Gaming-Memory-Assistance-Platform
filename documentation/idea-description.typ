...
#set page(
  ...
  footer: context {
    if counter(page).get().first() > 1 [
      #set text(9pt, fill: rgb("#888888"))
      #align(center)[CogniCare CDTx — Idea Description — SIH26003 · Page #counter(page).display("1")]
    ]
  },
)
...
#let h1(body) = {
  block(below: 0.55em, above: 1.1em)[
...
#let h2caption(body) = {
  block(below: 0.8em)[
    #set text(size: 10.5pt, weight: "bold", style: "italic", fill: rgb("#555555"))
    #text(body)
  ]
}

#let bullets(xs) = {
  list(xs, marker: [—], spacing: 0.35em)
}

#let numbered(xs) = {
  enum(xs, spacing: 0.35em)
}
...
#align(center)[
  ...
  #block[
    ...
    An AI-Based Cognitive Gaming & Memory Assistance Platform for Elderly Dementia Patients in the North Eastern Region
  ]
  #v(0.5em)
  ...
  #block[
    ...
    *Idea Description* · Smart India Hackathon
  ]
  #v(0.15em)
  #block[
    #set text(size: 10pt, fill: rgb("#444444"))
    Problem Statement #text(fill: brand, weight: "bold")[SIH26003] · MDoNER · Software · MedTech / BioTech / HealthTech
  ]
  #v(0.4em)
]
...
#h1[2. Problem Deep-Dive and Regional Context]
...
#h1[3. The Solution: CogniCare CDTx]
...
#h1[4. The Cognitive Gaming Suite — Meeting Requirement (a)]
...
#h1[5. Adaptive AI Difficulty Engine — Meeting Requirement (b)]
...
#h1[6. Multilingual and Voice-Assisted Interaction — Meeting Requirement (c)]
...
#h1[7. Cultural Anchoring in NER — Meeting Requirement (d)]
...
#h1[8. Daily Living Assistance & Reminders — Meeting Requirement (e)]
...
#h1[9. Caregiver Monitoring & Dashboards — Meeting Requirement (f)]
...
#h1[10. Offline-First, Low-Connectivity Architecture — Meeting Requirement (g)]
...
#h1[11. Elderly-Friendly Mobile/Tablet Interface — Meeting Requirement (h)]
...
#h1[12. Patient-Data Security & Privacy]
...
#table(
  ...
  align: (left, left),
  inset: 0.5em,
  table.header([*Layer*], [*Approach*]),
  ..(techrows.map(r => (text(weight: "bold", size: 9.5pt, r.at(0)), text(size: 9.5pt, r.at(1)))).flatten()),
)
...
#h1[14. Innovation: What Makes This Different]
...
#h1[15. Feasibility & Execution Plan]
...
#table(
  ...
  align: (left, left),
  inset: 0.5em,
  table.header([*Phase*], [*Focus*]),
  ..(phaserows.map(r => (text(weight: "bold", size: 9.5pt, r.at(0)), text(size: 9.5pt, r.at(1)))).flatten()),
)
...
#h1[16. Scalability & Adoption Roadmap for MDoNER]
